...
public class LoginSignupController {
    ...
    public ResponseEntity<LoginSignup> registerUser(@RequestBody LoginSignup user) {
    	LoginSignup createdUser = userService.createUser(user.getName(), user.getEmail(), user.getPassword());
        return new ResponseEntity<>(createdUser, HttpStatus.CREATED);
    }

    @PostMapping("/login")
    public ResponseEntity<Boolean> loginUser(@RequestBody LoginSignup user) {
